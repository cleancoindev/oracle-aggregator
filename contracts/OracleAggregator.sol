...
contract GelatoOracleAggregator is Ownable {
  ...
  event ReturnAmount(uint256 returnAmount, uint256 nrOfDecimals);
  ...
  function getExpectedReturnAmount(
    ...
    require(
      ...
      "Cannot have two same tokens as input"
    );
    require(
      (isUSD[tokenAddress_a] && isUSD[tokenAddress_b]) != true,
      "Cannot have two USD tokens as input"
    );

    uint256 returnRate_a;
    uint256 returnRate_b;
    uint256 nrOfDecimals_a;
    ...
    if (tokenAddress_b == ETH_ADDRESS || tokenAddress_b == USD_ADDRESS) {
      ...
      if (tokenPairAddress[tokenAddress_a][tokenAddress_b] != address(0)) {
        (returnRate_a, nrOfDecimals_a) = getRate(
          tokenAddress_a,
          tokenAddress_b
        );
        ...
        returnAmount =
          (amount * (returnRate_a * 10**nrOfDecimals_a)) /
          returnRate_b;
      ...
      if (pair_a == pair_b) {
        ...
        (returnRate_a, nrOfDecimals_a) = getRate(tokenAddress_a, pair_a);
        (returnRate_b, ) = getRate(tokenAddress_b, pair_b);
        (uint256 returnRate_ETHUSD, uint256 nrOfDecimals_ETHUSD) =
    ...
    returns (uint256, uint256)
  {
    ...
    uint256 nrOfDecimals = priceFeed.decimals();
