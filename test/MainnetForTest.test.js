...
describe("GelatoOracleAggregator TEST", async function () {
  ...
  it("should add new WBTC/USD token pair", async () => {
    ...
    const res = await contract.getExpectedReturnAmount(
      25,
      WBTC_ADDRESS,
      USD_ADDRESS
    );
  ...
  it("should be reverted when input has two same tokens", async () => {
    await expect(contract.getExpectedReturnAmount(25, KNC_ADDRESS, KNC_ADDRESS))
      .to.be.reverted;
  });
  ...
  it("should get expected return amount of USDC/AAVE", async () => {
    ...
    const ethUsdChainlinkOracle = await ethers.getContractAt(
      ChainlinkOracleAbi,
    ...
    const oraclePriceAave =
      parseInt(oracleDataAave.answer) / parseInt(oracleDecimalsAave);

    const oracleDataEth = await ethUsdChainlinkOracle.latestRoundData();
    const oracleDecimalsEth = await ethUsdChainlinkOracle.decimals();
    const oraclePriceEth =
      parseInt(oracleDataEth.answer) / parseInt(oracleDecimalsEth);

    // Desired Return Amount
    const desiredReturnAmount =
      parseInt(sellAmount) * (oraclePriceAave / oraclePriceEth);

    console.log("returnAmount: ", returnAmount / Math.pow(10, nrOfDecimals));
    console.log("returnAmountUnits: ", returnAmount);
    // Exchange Rate confirmed via CoinGecko
    console.log(`1 Aave is worth ${oraclePriceAave / oraclePriceEth} USDC`);
  ...
  it("should be reverted when input has invalid token address", async () => {
    await expect(
      contract.getExpectedReturnAmount(
        1,
        "0x8CE9137d39326AD0cD6491fb5CC0CbA0e089b6A8",
        KNC_ADDRESS
      )
    ).to.be.reverted;
  });
...
describe("GelatoOracleAggregator DEMO", async function () {
  ...
  it("Get return amount between token pairs", async () => {
    try {
      ...
      console.log(
        `🌟Return Amount of tokenA/tokenB with an amount of ${
          amount / Math.pow(10, amount_tenToPower)
        }: ` + returnAmount
      );
    } catch (err) {
